...
1. **Plugin MCP is OAuth URL-only** — `plugin.json` `mcpServers` must be the string `"./mcp.json"`, not an inline object. Plugin `mcp.json` must not contain `Authorization` or `${…}`. Device Code still writes `~/.cursor/mcp.json`.
